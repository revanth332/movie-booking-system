...
import {
  ...
  getTheaters,
  getBookings,
  getTheaterTimeMovieId,
  getSeats,
  getMoviesByGenre,
} from "../../src/app/v1/controllers/user.controller.js";
...
describe("user conroller", () => {
  ...
  describe("movie booking", () => {
    ...
    beforeEach(() => {
      mockReq = {
        body: {
          userId: "user1",
          seats: ["seat1", "seat2"],
          theaterTimeMovieId: "theatertimemovieid",
        },
      };
    ...
    it("should book movie successfully", async () => {
      jest
        .spyOn(User, "bookMovie")
        .mockResolvedValueOnce({
          status: StatusCodes.OK,
          msg: "Successfully booked movie",
        });

      await bookMovie(mockReq, mockRes);

      expect(User.bookMovie).toHaveBeenCalledWith({
        userId: "user1",
        seats: ["seat1", "seat2"],
        theaterTimeMovieId: "theatertimemovieid",
      });
  ...
  describe("cancelBooking", () => {
    ...
    it("should cancel booking successfully", async () => {
      jest
        .spyOn(User, "cancelBooking")
        .mockResolvedValueOnce({
          status: StatusCodes.OK,
          msg: "Successfully cancelled movie",
        });

      await cancelBooking(mockReq, mockRes);

      expect(User.cancelBooking).toHaveBeenCalledWith({
        bookingId: "bookingid",
      });
      expect(mockRes.status).toHaveBeenCalledWith(200);
  ...
  describe("getShowTimes from theater movie id", () => {
    ...
    it("should return show times successfully", async () => {
      jest
        .spyOn(User, "getShowTimes")
        .mockResolvedValueOnce({ status: StatusCodes.OK, data: [mockShow] });
  ...
  describe("fetching Theaters", () => {
    ...
    it("should fetch theaters sucessfully based on movie Id", async () => {
      jest
        .spyOn(User, "getTheaters")
        .mockResolvedValueOnce({ status: StatusCodes.OK, data: [mockTheater] });
  ...
  describe("fetching Bookings", () => {
    ...
    it("should fetch bookings based on userid", async () => {
      jest
        .spyOn(User, "getBookings")
        .mockResolvedValueOnce({ status: StatusCodes.OK, data: [mockBooking] });
  ...
  describe("fetching Theater Movie Ids", () => {
    ...
    it("should fetch Theater Movie Id based on theater movie id and time", async () => {
      jest
        ...
        .mockResolvedValueOnce({
          status: StatusCodes.OK,
          data: [{ theaterTimeMovieId: "theatermovietime123" }],
        });
      ...
      expect(mockRes.send).toHaveBeenCalledWith([
        { theaterTimeMovieId: "theatermovietime123" },
      ]);
    });
  ...
  describe("fetching seat details", () => {
    ...
    it("should fetch seat details successfully", async () => {
      jest
        .spyOn(User, "getSeats")
        .mockResolvedValueOnce({ status: StatusCodes.OK, data: [mockSeat] });
